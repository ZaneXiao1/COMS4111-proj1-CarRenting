...
def search():
	...
	s = 'SELECT DISTINCT model FROM car_type CT'
	if office != 'none' and office != None:
		s += " WHERE EXISTS (SELECT * FROM car C WHERE C.model=CT.model AND C.office_name='" + office + "')"
	...
	if request.method=='POST':
		search_result.append(['car_id', 'color', 'mileage', 'model', 'location', 'brand', 'function', 'capacity', 'price/day($)', 'rating'])
		s = 'SELECT C.*, brand, function, capacity, unit_time_price, rating FROM car C, car_type CT WHERE C.model=CT.model'
		# constraint office
		if office != 'none' and office != None:
	...
	context = dict(results=search_result, offices=off_result, models=model_result, now=now, **params)
